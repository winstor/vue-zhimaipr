<template>
		...
		>
				...
				<el-table-column :title="22" align="center" label="ID" width="100">
				...
				/>
				<el-table-column  v-if="actionData" align="center" :label="actionData.label || '操作'" :width="actionData.width || 300">
						<template slot-scope="scope">
								<el-button
												v-for="button in actionData.buttons"
												v-if="checkAction(scope,button.value)"
												@click="handleAction(scope,button.value)"
								...
								<!--<el-button v-if="" type="danger" size="small" @click="handleDelete(scope)">删除</el-button>-->
						</template>
				</el-table-column>
		</el-table>
</template>
        ...
        methods: {
            ...
            handleDelete(scope) {
                ...
                this.$confirm('确定删除吗?', 'Warning', {
                    ...
                    this.$parent.destroy(scope.row.id)
                }).catch(err => {
										console.log("取消删除")
                })
            },
