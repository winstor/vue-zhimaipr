		...
		>
				...
				<el-table-column align="center"
						...
						<template slot-scope="scope">
								...
								<el-button v-if="!actionData.disable || actionData.disable.indexOf('show') === -1"
								           type="success"
								           size="small"
								           @click="handleAction(scope,'show')">查看
								</el-button>
								<el-button v-if="!actionData.disable || actionData.disable.indexOf('edit') === -1"
								           type="primary"
								           size="small"
								           @click="handleAction(scope,'edit')">编辑
								</el-button>
								<el-button
												v-if="!actionData.disable || actionData.disable.indexOf('delete') === -1"
												type="danger"
												size="small"
												@click="handleAction(scope,'delete')"
								>删除
								</el-button>
								<el-button v-for="(button,index) in actionData.appends"
								           :type="button.type || 'success'"
								           :size="button.size || 'small'"
								           @click="handleAction(scope,index)">{{button.label}}
								</el-button>
